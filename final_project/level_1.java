...
public class level_1 implements Initializable{
	...
	public void initialize(java.net.URL arg0, ResourceBundle arg1) {
		...
		for(int i = 0;i<levelDetail.level1BricksNumber;i++)
			brickList.add(new brick(levelDetail.level1BricksX[i],levelDetail.level1BricksY[i]));
		for(int i=0;i<levelDetail.level1BricksNumber;i++) BrickPane.getChildren().add(brickList.get(i).brick);
		newBall=new ball();
	...
	private void setNextFrame() {
		...
		if(isdie) {
			...
			try {
				...
				controller.passValue(1,totalScore, HP,bigTimer.get(),false);
	...
	public void win() throws IOException {
		...
		resultController controller=loader.getController();
		controller.passValue(1,totalScore, HP,bigTimer.get(),true);
		Scene resultScene=new Scene(result);
		resultScene.getRoot().requestFocus();
		welcome.currentStage.setScene(resultScene);
	}
