...
public class level_2 implements Initializable{
	ArrayList<brick> brickList=new ArrayList<>();
	ArrayList<ImageView> HPList=new ArrayList<>();
//	1.ball巨大化 2.ball變慢 3.分數加倍 4.hp增加
	...
	public void initialize(java.net.URL arg0, ResourceBundle arg1) {
		...
		for(int i = 0;i<levelDetail.level2BricksNumber;i++)
			brickList.add(new brick(levelDetail.level2BricksX[i],levelDetail.level2BricksY[i]));
		for(int i=0;i<levelDetail.level2BricksNumber;i++) BrickPane.getChildren().add(brickList.get(i).brick);
		newBall=new ball();
	...
	private void setNextFrame() {
		...
		if(isdie) {
			...
			try {
				...
				controller.passValue(2,totalScore, HP,bigTimer.get(),false);
	...
	public void win() throws IOException {
		...
		resultController controller=loader.getController();
		controller.passValue(2,totalScore, HP,bigTimer.get(),true);
		Scene resultScene=new Scene(result);
		resultScene.getRoot().requestFocus();
		welcome.currentStage.setScene(resultScene);
	}
