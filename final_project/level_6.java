...
public class level_6 implements Initializable{
	...
	public void initialize(java.net.URL arg0, ResourceBundle arg1) {
		...
		for(int i = 0;i<levelDetail.level6BricksNumber;i++)
			brickList.add(new brick(levelDetail.level6BricksX[i],levelDetail.level6BricksY[i]));
		for(int i=0;i<levelDetail.level6BricksNumber;i++) BrickPane.getChildren().add(brickList.get(i).brick);
		newBall=new ball();
	...
	private void setNextFrame() {
		...
		if(isdie) {
			...
			try {
				...
				controller.passValue(6,totalScore, HP,bigTimer.get(),false);
	...
	public void win() throws IOException {
		...
		resultController controller=loader.getController();
		controller.passValue(6,totalScore, HP,bigTimer.get(),true);
		Scene resultScene=new Scene(result);
		resultScene.getRoot().requestFocus();
		welcome.currentStage.setScene(resultScene);
	}
