...
public class level_10 implements Initializable{
	...
	public void initialize(java.net.URL arg0, ResourceBundle arg1) {
		...
		for(int i = 0;i<levelDetail.level10BricksNumber;i++)
			brickList.add(new brick(levelDetail.level10BricksX[i],levelDetail.level10BricksY[i]));
		for(int i=0;i<levelDetail.level10BricksNumber;i++) BrickPane.getChildren().add(brickList.get(i).brick);
		newBall=new ball();
	...
	private void setNextFrame() {
		...
		if(isdie) {
			...
			try {
				...
				controller.passValue(10,totalScore, HP,bigTimer.get(),false);
	...
	public void win() throws IOException {
		...
		resultController controller=loader.getController();
		controller.passValue(10,totalScore, HP,bigTimer.get(),true);
		Scene resultScene=new Scene(result);
		resultScene.getRoot().requestFocus();
		welcome.currentStage.setScene(resultScene);
	}
